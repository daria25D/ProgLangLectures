...
```
То, что является данными, может являться результатом выолнения каких-то операций.
...
Функции в Java захватывают переменные только по значению, так как изменение значений может быть небезопасно при многопоточности.
**closure**
